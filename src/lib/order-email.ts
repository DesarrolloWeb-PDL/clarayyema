...
function getFromAddress() {
  return process.env.ORDER_EMAIL_FROM?.trim() || 'Clara y Yema <onboarding@resend.dev>'
}
...
function renderCustomerEmailHtml(order: OrderEmailPayload, contactEmail: string) {
  return `
    <div style="font-family:Arial,sans-serif;max-width:640px;margin:0 auto;padding:24px;color:#111827;">
      <h1 style="margin:0 0 16px;font-size:28px;color:#92400e;">Clara y Yema</h1>
...
export async function sendOrderPaidEmails(order: OrderEmailPayload): Promise<SendOrderPaidEmailsResult> {
  ...
  const customerResponse = await resend.emails.send({
    ...
    subject: `Pedido confirmado ${order.orderNumber} | Clara y Yema`,
